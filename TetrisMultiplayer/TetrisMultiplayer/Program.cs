    ...
    static async Task<List<Networking.NetworkManager.PlacedPieceMsg>> WaitForPlacedPieces(NetworkManager network, List<string> playerIds, int timeoutMs, ILogger logger, CancellationToken cancellationToken, Dictionary<string, int>? hps = null, HashSet<string>? spectators = null, HashSet<string>? playersWhoPlaced = null)
        {
            ...
            while ((DateTime.UtcNow - start).TotalMilliseconds < adaptiveTimeout && received.Count < activePlayers.Count && !cancellationToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    if (msg != null && !received.Contains(msg.PlayerId) && activePlayers.Contains(msg.PlayerId))
                    {
                        ...
                        // Extend timeout slightly if we're getting responses
                        if (received.Count < activePlayers.Count)
                        {
                            adaptiveTimeout = Math.Min(adaptiveTimeout + 2000, timeoutMs);
                        }
                    }
                }
                catch (Exception ex)
            ...
            {
                logger.LogWarning($"[Host] Timeout: Fehlende PlacedPiece von {missingPlayers.Count} Spielern: {string.Join(", ", missingPlayers)}");
                
                // Handle timeouts more aggressively
            ...
            // Eliminate players after fewer missed rounds (2 instead of 3)
            if (hps != null && spectators != null)
            {
                foreach (var id in missingPlayers)
                {
                    ...
                    {
                        logger.LogWarning($"[Host] Spieler {id} nach 2 verpassten Runden eliminiert (Disconnect/Timeout)");
                        hps[id] = 0;
                        spectators.Add(id);
                    }
                }
